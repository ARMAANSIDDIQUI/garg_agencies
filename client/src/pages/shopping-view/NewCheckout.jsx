...
function ShoppingCheckout() {
  ...
  const totalCartAmount =
    cartItems && cartItems.items && cartItems.items.length > 0
      ...
      function handlePlaceOrder() {
        ...
        // Check if running in Flutter WebView (i.e., mobile app)
        if (typeof window.isInFlutterWebView !== 'undefined' && typeof window.locationFromApp !== 'undefined') {
          window.locationFromApp(
            (position) => {
              const { latitude, longitude } = position.coords;
              placeOrder(latitude, longitude); // Call the function to place the order
            },
            () => {
              toast({
                title: "Unable to retrieve your location from the app.",
                variant: "destructive",
              });
            }
          );
  ...
  function placeOrder(latitude, longitude) {
